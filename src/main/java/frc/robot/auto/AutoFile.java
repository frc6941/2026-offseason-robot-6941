...
public class AutoFile {
  ...
  private static Command buildSweepShootCycle(String sweepPathName, boolean isLeft) {
    return Commands.sequence(
        ...
        Commands.runOnce(() -> {})
            .withTimeout(0.1)
            .deadlineFor(
                shooterDefault(), shootingSuperstructure.getIdx().runState(() -> IdxMode.OFF)));
  }
    ...
    return Commands.parallel(
            ...
            Commands.sequence(
                ...
                Commands.parallel(
                        ...
                        Commands.sequence(
                            Commands.deadline(
                                new WaitUntilCommand(() -> autoTimer.get() >= 17),
                        ...
                        new WaitUntilCommand(
                            ...
                            .andThen(
                                Commands.parallel(
                                    Commands.defer(intake::zeroCommand, Collections.emptySet()),
                                    intake())))
                ...
                Commands.sequence(
                        ...
                        drivePastSlope(isLeft, false),
                        Commands.deadline(followPathFile("rightIntake", isLeft), intake()))
                    .onlyIf(() -> endBehaviourChooser.get() == EndBehaviour.INTAKE)))
  ...
  private static Command buildHunt() {
    ...
    Command hunt = deadline(followPathFile("huntRight", isLeft), intake(), shootBackWhenSuitable());
    ...
    var resetWhenNeeded =
        waitUntil(() -> isLeft && DriverStation.isAutonomous() && autoTimer.get() >= 18)
            .andThen(parallel(defer(intake::zeroCommand, Collections.emptySet()), intake()));
    return sequence(
            driveToNeutral, //
            hunt, //
            driveBack, //
            parallel( //
                sequence(driveToCorner, waitSeconds(1.5), sweepToClimb), resetWhenNeeded))
        .withInterruptBehavior(Command.InterruptionBehavior.kCancelIncoming);
  }
