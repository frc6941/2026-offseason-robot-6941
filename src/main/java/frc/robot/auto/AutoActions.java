...
public class AutoActions {
    ...
    return swerve.defer(
        () -> {
          ...
          return Commands.deadline(waitCrossedBump(isToNeutral), driveToPose(targetPose))
              .beforeStarting(() -> Logger.recordOutput("Temp/isPitchStable", isPitchStable()));
        });
  }
  ...
  public static Command allignToClimb(boolean isLeft) {
    return swerve.defer(
        () -> {
          ...
          return driveToAllign(climbPose, AutoActions::getShiftDirectionTowardBump, 1.2, 0.2);
        });
  }
  ...
  public static boolean hasCrossedBump(boolean isToNeutral) {
    return isToNeutral
        ? AllianceFlipUtil.applyX(getRobotX()) > FieldConstants.LinesVertical.neutralZoneNear
  ...
  public static Command simpleClimb() {
    return climber
        .runMotionMagic(Meters.of(ClimberParamsNT.climbReadyMeters.getValue()))
        .until(climber::positionAtGoal)
        .andThen(climber.runMotionMagic(Meters.of(ClimberParamsNT.climbedMeters.getValue())));
  }
        ...
        .beforeStarting(
            Commands.runOnce(
                () -> {
                  ...
                  swerve.setSwerveModuleLimit(SwerveMK5Config.kShootingSwerveLimit);
                }))
  ...
  public static Command oscillateIntakeFeed() {
    return Commands.defer(
        () -> {
          double feedOscillationRateHz = IntakerExtensionParamsNT.feedOscillationRateHz.getValue();
  ...
  public static Command shooterDefault() {
    return Commands.defer(
        () ->
            shooter.runVelVolt(() -> RotationsPerSecond.of(ShooterParamsNT.idleVelRPS.getValue())),
        Collections.emptySet());
  }
  ...
  public static Command testPath() {
    return swerve.defer(
        () -> {
          ...
          RotationTarget rotationTargetA = new RotationTarget(kTestRotationAPose, kTestRotationA);
          RotationTarget rotationTargetB = new RotationTarget(kTestRotationBPose, kTestRotationB);
          List<Pose2d> waypoints =
              List.of(
                  current,
          ...
          List<RotationTarget> rotationTargets = List.of(rotationTargetA, rotationTargetB);
          PathPlannerPath path = generatePath(waypoints, Collections.emptyList(), 4.2, 6.0, 0.0);
            ...
            swerve.setSwerveLimit(
                SwerveLimit.builder()
                    ...
                    .maxAngularAcceleration(
                        DegreesPerSecondPerSecond.of(maxAngularAccelerationDegps2))
                    .build()));
  }
