...
public class IntakerSubsystem extends SubsystemBase {
  ...
  public void setDefaultCommand() {
    ...
    extension.setDefaultCommand(
        extension.runPosition(
            () ->
                switch (currentMode) {
                  case INTAKING -> Meters.of(IntakerExtensionParamsNT.deployPosMeters.getValue());
                  case EXTENDED_IDLE, EXTENDED_REVERSE ->
                      Meters.of(IntakerExtensionParamsNT.deployPosMeters.getValue());
                  case RETRACTED -> Meters.of(IntakerExtensionParamsNT.retractPosMeters.getValue());
                  case FEEDING -> Meters.of(IntakerExtensionParamsNT.feedPosMeters.getValue());
                  default -> Meters.of(IntakerExtensionParamsNT.retractPosMeters.getValue());
                }));
  }
  ...
  public Command outZeroCommand() {
    double zeroVoltage =
        MathUtil.clamp(
            -IntakeConfig.INTAKER_EXTENSION_CONFIG.zeroingConfig.zeroingVoltage, -12.0d, 12.0d);
                ...
                () -> {
                  ...
                  currentFilter =
                      LinearFilter.movingAverage(
                          IntakeConfig.INTAKER_EXTENSION_CONFIG.zeroingConfig.zeroingFilterSize);
            ...
            .andThen(
                Commands.deadline(
                                ...
                                () ->
                                    currentFilterValue =
                                        currentFilter.calculate(
                                            extension.getStatorCurrent().in(Amps)))
                            .until(
                                () ->
                    ...
                    .finallyDo(
                        interrupted -> {
                          if (!interrupted) {
                            extension.setCurrPos(Meters.of(is10541 ? 0.315766 : 0.319718));
                          }
                        })
    ...
    Command simZero =
        Commands.sequence(
            Commands.runOnce(() -> extension.setCurrPos(Meters.of(is10541 ? 0.315766 : 0.319718))),
            new WaitCommand(0.2),
            Commands.runOnce(
                () ->
  ...
  public void periodic() {
    ...
    if (!autoOutZeroRunning && zeroTimer.hasElapsed(2.0)) {
      ...
      CommandScheduler.getInstance()
          .schedule(
              outZeroCommand().withInterruptBehavior(Command.InterruptionBehavior.kCancelSelf));
      zeroTimer.restart();
    }
  }
