...
public class TurretSubsystem extends VelocityMotorSubsystem<MotorInputsAutoLogged, MotorIO> {
    ...
    protected void logState() {
        ...
        Logger.recordOutput(getName() + "/currentPoseWorld", getCurrentTurretPoseWorld());
        Logger.recordOutput(getName() + "/targetPoseWorld", getTargetTurretPoseWorld());
    }
    ...
    public Command runTurretTargetLoop() {
        return Commands.runOnce(
                        () -> {
                            posVelCtl.reset(getPosition().in(Degrees));
                            targetAngleWorld = () -> Degrees.of(0.0);
                        })
    ...
    private AngularVelocity calculateTargetVelocity(Angle targetAngle) {
        ...
        double chassisCmdOmegaDegPerSec =
    ...
    private Pose2d getTargetTurretPoseWorld() {
        ...
        return new Pose2d(
                ...
                Rotation2d.fromDegrees(targetAngleWorld.get().in(Degrees)));
    }
    ...
    private void updateUnwrappedTurretAngle() {
        ...
        Logger.recordOutput("Turret/unwrappedTurretAngle", unwrappedTurretAngle);
    }
