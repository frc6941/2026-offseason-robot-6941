...
public class ShootingSuperstructure {
    ...
    hood.setDefaultCommand(
        hood.runPosition(
            () ->
                Degrees.of(
                    MathUtil.clamp(
                        RobotStateRecorder.getCmdFrame().hoodAngle().in(Degrees), 0, 51))));
  }
                ...
                idx.runState(
                    () ->
                        turret.getCurrentMode() == TurretMode.TRACKING
                            ? forceFeed ? IdxMode.FORCE_FEED : IdxMode.FEED
                            : IdxMode.OFF))
  ...
  public Command shootWhenReady(boolean forceFeed, GenericHID... hids) {
    return Commands.parallel(
        ...
        new RumbleWhenCommand(
            () -> (mode == TargetMode.GOAL && !HubShiftUtil.getShiftedShiftInfo().active()), hids),
        Commands.waitUntil(shooter::velocityAtGoal)
            .andThen(
                Commands.runOnce(() -> isShooting = true),
                idx.runState(
  ...
  public Command runFrame() {
    return Commands.parallel(
        turret.setTurretPoseWorld(() -> RobotStateRecorder.getCmdFrame().turretAngleWorld()),
        hood.runPosition(
            () ->
                Degrees.of(
                    MathUtil.clamp(
  ...
  private double computeRpm(LinearVelocity muzzleSpeed, Angle bba) {
    ...
    if (currentDistance > 2.5) {
      ...
      if (currentDistance >= maxDistance) {
        ...
        scalingFactor = 1.0 + (ShotCalculatorParamsNT.distanceScaler.getValue() - 1.0) * t_distance;
      }
    }
  ...
  public double getDistance() {
    ...
    if (mode == TargetMode.GOAL) {
      return RobotStateRecorder.getTranslationShotToTargetCurrent(RobotStateRecorder.kFrameGoal)
          .getNorm();
    } else {
