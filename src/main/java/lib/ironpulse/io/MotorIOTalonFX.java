...
public class MotorIOTalonFX implements MotorIO {
  ...
  public MotorIOTalonFX(SubsystemConfig cfg) {
    ...
    fx.MotorOutput.NeutralMode = cfg.defaultBrake ? NeutralModeValue.Brake : NeutralModeValue.Coast;
    ...
    if (!Double.isNaN(cfg.forwardSoftLimitDegrees.magnitude())) {
      ...
      fx.SoftwareLimitSwitch.ForwardSoftLimitThreshold = cfg.forwardSoftLimitDegrees.in(Rotations);
    } else {
    ...
    if (!Double.isNaN(cfg.reverseSoftLimitDegrees.magnitude())) {
      ...
      fx.SoftwareLimitSwitch.ReverseSoftLimitThreshold = cfg.reverseSoftLimitDegrees.in(Rotations);
    } else {
    ...
    for (int i = 0; i < followers.length; i++) {
      ...
      if (!Double.isNaN(f.statorCurrentLimitAmps) || !Double.isNaN(f.supplyCurrentLimitAmps)) {
        ...
        if (!Double.isNaN(f.ramp)) {
          followers[i]
              .getConfigurator()
              .apply(new ClosedLoopRampsConfigs().withVoltageClosedLoopRampPeriod(f.ramp));
        }
    ...
    signals =
        new BaseStatusSignal[] {posSig, velSig, motorVoltSig, supplyVoltSig, statorSig, supplySig};
  ...
  public void setVelTCSetpoint(AngularVelocity velocity, Current feedForwardTorqueCurrent) {
    main.setControl(
        velocityTorqueCurrentCtrl.withVelocity(velocity).withFeedForward(feedForwardTorqueCurrent));
  }
