package lib.ironpulse.indicator.led;

import edu.wpi.first.wpilibj.AddressableLED;
import edu.wpi.first.wpilibj.AddressableLEDBuffer;
import edu.wpi.first.wpilibj.Notifier;
import edu.wpi.first.wpilibj.util.Color;
import lib.ironpulse.indicator.led.patterns.SolidColorPattern;
...
public class AddressableLEDWrapper {
    ...
    private final LoggedNetworkString dashboardBuffer =
            new LoggedNetworkString("/SmartDashboard/Buffer");

    // Current pattern applied to the LED strip
    @Setter @Getter private AddressableLEDPattern pattern = new SolidColorPattern(Color.kBlack);
    // Intensity level for the LED strip, values range from 0.0 to 1.0
    @Setter private double intensity = 0.1;
