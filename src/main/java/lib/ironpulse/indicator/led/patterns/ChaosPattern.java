...
public class ChaosPattern implements AddressableLEDPattern {
    ...
    // Sets the LEDs to a random color initially and then shifts their colors randomly in subsequent
    // calls
    @Override
    ...
    private Color randomColorShift(Color color) {
        return new Color(randomShift(color.red), randomShift(color.green), randomShift(color.blue));
    }

    // Shifts a color component value by a random amount in either direction, ensuring the value
